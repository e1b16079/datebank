...
def main():
  ...
  for j in range(1, imh-1):     #1～imh-1まで繰り返し
    for i in range(1, imw-1):   #1～imw-1まで繰り返し
      res[j, i] =(gray[j+1,i-1]+2*gray[j+1,i]+gray[j+1,i+1]+2*gray[j,i-1]+4*gray[j,i]+2*gray[j,i+1]+gray[j-1,i-1]+2*gray[j-1,i]+gray[j-1,i+1])/16  #(gray[j, i]とその8近傍の画素の合計値)/9

  cv2.imshow("code 6", res)
  cv2.imwrite("code6-1.bmp", res)
  cv2.waitKey()
